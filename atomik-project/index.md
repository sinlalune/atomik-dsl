...
Language **v0.3.1** (surface `atomik 0.3` unchanged) · Scene IR **0.1** · `cycle` archetype end-to-end (prototype in `apps/prototype-cycle/`) · `flow` archetype end-to-end (ranks, determinism, lane bands painted, routed back-edges, contractual `cycle`→`flow` fallback, prototype flip, typed) · **65/65 tests** green including golden-fixture byte parity and the A1–A7 runtime oracle · errata **C1–C4 folded** (language spec §14 changelog; render-core §10 marked, §6.2 cycle implementation notes) · kernel split `lang.js`/`render.js`/`index.js` with typed surface (`index.d.ts`), public API unchanged · **CP-DSL-001 and CP-DSL-002 both closed 2026-07-07** · active path: **CP-DSL-003** (D3 — generability evaluation; scope amended 2026-07-07 to **two subject models — Haiku 4.5 + Gemini 3.1 Flash-Lite** — Sonnet 5 judge, full 16×5×2 matrix per provider over each vendor's Batch API). Corpus (16 tasks) + multi-provider kernel-as-grader harness (`apps/eval-generability/`; 36 self-tests, dry-run proven, Gemini adapter offline-verified) built through S03; **S04 execution blocked on owner-provided `ANTHROPIC_API_KEY` + `GEMINI_API_KEY`** · **D4** executes in the main repo after its S00 owner amendment.
