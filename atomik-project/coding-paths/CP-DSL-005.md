...
atomik:
  id: CP-DSL-005
  status: active
  note: reopened for ongoing owner-driven demo iteration (S06+); the S05 core is done, the demo is a living dev tool
  current_step: iterating
  base_commit: 6d777be
---
...
- [x] S09 (owner request) **Show + edit the prompt in the UI.** New `POST /api/prompt` composes (without running) the exact `{system, user}`; `/api/generate` accepts an optional `prompt` and uses it verbatim (skipping composeItem), returning the prompt actually sent; `/api/save` records the used prompt (edited or composed). UI: a collapsible **Prompt** panel with editable system + user textareas, `↻ Load default`, and a **use this edited prompt** checkbox; a normal generation back-fills the panel with what was sent, a reload restores a saved run's prompt. Lets the owner test prompt/spec changes live without touching files. Verified: /api/prompt returns the pocket-spec system + STRICT-SYNTAX user; a custom prompt is echoed and used verbatim; normal generate reflects the composed prompt. Demo-local; kernels untouched. (Also noted: local-model support via llama.cpp/Ollama is paused pending the owner's own llama.cpp CLI results — the provider would be a ~40-line OpenAI-compatible fetch when resumed.)
...
- [x] S10 (owner request) **System-prompt preset selector — Minimal ↔ Guided A/B.** Owner: reading the terse pocket-spec prompt, "there is no full semantical explanation of what makes atomik" — so a second, *guided* system prompt was authored (`apps/generate-demo/prompts/guided.md`): it contextualizes the four planes, glosses **every** statement / effect / closed-vocabulary value one line each, and states the generation rules explicitly. Its rule 8 was **corrected on owner feedback**: not "include only what the SOURCE states" (wrong for the real use case — highlight one *term* → ask the AI for a representation) but "match scope to input — expand from well-established knowledge for a seed/term, stay epistemically honest, never fabricate specifics (dates, sources, `[[links]]`)"; a seed-mode example was added. The demo gains a `genpreset` dropdown wiring `preset` through `/api/prompt`, `/api/generate`, `/api/save`; `composeItem(text, teaching, preset)` swaps **only** the system prompt (user instruction + hardening rules + source held constant → a clean A/B). The preset is echoed by `/api/generate`, recorded in saved runs, and restored on reload; switching it live-refreshes the (unedited) prompt panel. Verified end-to-end (both presets, Flash-Lite): a single-word seed "Photosynthesis" expands to a full 5–6-node scene, 0 errors, under both prompts; save records `preset:"guided"` with the guided system text. Demo-local — the eval's measured prompt (pocket spec) stays frozen; kernels untouched.
