...
See `apps/prototype-cycle/atomik_ui.js` (paint + interaction over the kernel) and the presets in `fixtures/presets/`. `apps/eval-generability/` (CP-DSL-003) is a second read-only consumer: it uses `parse` under the `generated` profile with a vault-backed `resolver` as an automated grader — the kernel scores model-generated scenes for validity, grounding, epistemic ceiling, and structural properties. It never imports internals and adds no dependency to `packages/dsl-core`.
