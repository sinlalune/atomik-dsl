...
See `apps/prototype-cycle/atomik_ui.js` (paint + interaction over the kernel) and the presets in `fixtures/presets/`. `apps/eval-generability/` (CP-DSL-003) is a second read-only consumer: it uses `parse` under the `generated` profile with a vault-backed `resolver` as an automated grader — the kernel scores model-generated scenes (from Haiku 4.5 and Gemini 3.1 Flash-Lite) for validity, grounding, epistemic ceiling, and structural properties. It never imports internals and adds no dependency to `packages/dsl-core`; the provider SDKs live behind that app's own dynamic imports. Results of the first run: `docs/evals/generability_eval_v0_3_1.md`. `apps/generate-demo/` (CP-DSL-005) is a third read-only consumer — a localhost server that generates a scene from pasted text and renders it through the prototype painter (`npm run demo`), the full generation→IR→layout→render chain end-to-end.
